...
fn add_gap_points(data: &[DataPoint], interval_ms: i64, freq: &str) -> Vec<DataPoint> {
    ...
    log_to_file(&format!(
        "add_gap_points START: {} raw points | freq={} | gap_threshold={} ms | max_fill={} ms",
        data.len(), freq, gap_threshold, max_gap_to_fill
    ));
    ...
    log_to_file(&format!(
        "add_gap_points FINISHED: {} total points ({} nulls inserted). Last real ts: {}",
        result.len(), null_count, last_ts
    ));
