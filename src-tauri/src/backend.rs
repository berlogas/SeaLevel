...
pub fn aggregate(
    ...
    // ✅ ИСПРАВЛЕНО: используем date_trunc как в Python (учитывает високосные годы, разную длину месяцев)
    let group_expr = match freq.as_str() {
        "10min"  => "CAST(epoch_ms(date_trunc('minute', EPOCH_MS(timestamp_ms))) / 600000 * 600000 AS BIGINT)",
        ...
        "decade" => "CAST(epoch_ms(date_trunc('year', EPOCH_MS(timestamp_ms))) / 10 * 10 AS BIGINT)",
        _        => "CAST(epoch_ms(date_trunc('day', EPOCH_MS(timestamp_ms))) AS BIGINT)",
    };

    let interval_ms = match freq.as_str() {
        "10min" => 600_000,
        "hour" => 3_600_000,
        "day" => 86_400_000,
        "week" => 604_800_000,
    ...
    let cached: Vec<DataPoint> = {
        ...
        stmt.query_map(params![&freq, start_ms, end_ms], |row: &Row| {
            Ok(DataPoint {
                datetime: String::new(),
                timestamp: row.get(0)?,
                mean: row.get(1).ok(),
    ...
    if !rows.is_empty() {
        ...
        for row in &rows {
            stmt.execute(params![
                &freq, row.timestamp, row.mean, row.std, row.min, row.max, row.count
            ]).map_err(|e: DbError| e.to_string())?;
        }
...
// ✅ ИСПРАВЛЕНО: маркер разрыва больше не использует timestamp: 0 (1970 год)
